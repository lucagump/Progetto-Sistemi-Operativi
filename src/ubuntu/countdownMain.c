...
/*Funzione che fa partire il programma permettendo all'utente di interagiretramite una shell interattiva*/
int main(int argc, char* argv[]){
	...
	setpgid(0,getpid());//setta il pid del processo main come group_pid di tutti si suoi figli
	...
	/*Controlli per funzione test o se viene utilizzata una sintassi errata*/
	if(argc==2){
		if(isNumber(argv[1])){
			startArgument=T;
			time=atoi(argv[1]);
	...
	/*Se gli argomenti in compilazione sono di più del previsto, genera errore*/
	if((pid=fork())==0){
